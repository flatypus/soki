...
export function Flashcard({ token }: FlashcardProps) {
  ...
  const fetchNextCard = useCallback(async () => {
    ...
    try {
      ...
      if (data.card) {
        ...
        toast({ title: "All Done!", description: data.message || "No more cards for now." });
      }
    } catch (err) {
      // Fix: Use Error type
      const errorMessage = err instanceof Error ? err.message : "Failed to load the next card.";
  ...
  const renderCardContent = () => {
    if (!currentCard) {
      return <p>No more cards for now. Check back later!</p>;
    }
      ...
      <>
        <CardHeader>
          ...
          <CardDescription className="text-center">
            ...
            {cardData.type === "kanji" && cardData.grade && ` (Grade ${cardData.grade})`}
          </CardDescription>
        </CardHeader>
              ...
              <div>
                ...
                <ul className="list-disc list-inside text-sm">
                  {cardData.readings.map((r, index) => (
                    <li key={index}>{r.reading} {r.word && r.word !== (cardData.type === "kanji" ? cardData.character : cardData.phrase) ? `(${r.word})` : ""}</li>
                  ))}
                </ul>
              </div>
              <div>
                ...
                <ul className="list-disc list-inside text-sm">
                  ...
                  {cardData.definitions.length > 5 && <li className="text-muted-foreground">...and more</li>}
                </ul>
              </div>
              {cardData.type === "kanji" && (
                 <div className="text-xs text-muted-foreground mt-2">
                    {cardData.strokeCount && `Strokes: ${cardData.strokeCount}`}
                    {cardData.frequency !== 99999 && cardData.frequency !== null && ` | Frequency Rank: ${cardData.frequency}`}
                 </div>
              )}
            </>
          ) : (
        ...
        <CardFooter className="flex justify-around">
          {isRevealed ? (
            <>
              ...
              <Button variant="destructive" onClick={() => submitReview(1)} disabled={isLoading}>Again (1)</Button>
              <Button variant="outline" onClick={() => submitReview(3)} disabled={isLoading}>Hard (3)</Button>
              <Button variant="outline" onClick={() => submitReview(4)} disabled={isLoading}>Good (4)</Button>
              <Button variant="default" onClick={() => submitReview(5)} disabled={isLoading}>Easy (5)</Button>
            </>
          ) : (
            <span className="text-muted-foreground">Reveal the answer to rate your recall.</span>
          )}
        </CardFooter>
      </>
    );
    ...
    return (
      <Card className="w-full">
        ...
        <CardContent>
          ...
          <Button onClick={fetchNextCard} className="mt-4">Try Again</Button>
        </CardContent>
      </Card>
    );
  }
